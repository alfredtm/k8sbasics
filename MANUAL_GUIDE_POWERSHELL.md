# Kubernetes Workshop: Manual Guide (PowerShell / Windows)

Step-by-step guide for the workshop without Claude Code, using PowerShell on Windows. You'll set up everything manually using `indev`, `oc`, and ArgoCD.
...
Look for a row containing `gitops` — it's ready when the phase shows `Succeeded`. Press `Ctrl+C` to stop watching.
...
Look for a row containing `cnpg` — it's ready when the phase shows `Succeeded`. Press `Ctrl+C` to stop watching.
...
It's ready when `SYNC STATUS` shows `Synced` and `HEALTH STATUS` shows `Healthy`. Press `Ctrl+C` to stop watching.
...
```powershell
$appUrl = oc get route todo-app -n todo-workshop -o jsonpath='{.spec.host}'
Write-Host "https://$appUrl"
Start-Process "https://$appUrl"
```

This opens the URL in your default browser. You should see the todo app with a **yellow banner**: **"Storage: In-Memory (ephemeral)"**.
...
When `STATUS` shows `Cluster in healthy state`, you're good. This may take a minute or two. Press `Ctrl+C` to stop watching.
